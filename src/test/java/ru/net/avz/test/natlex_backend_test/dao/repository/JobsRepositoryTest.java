...
public class JobsRepositoryTest {
    ...
    public void test() {
        ...
        TestHelper.runJobsTest(
                ...
                Pair.of(
                        job -> String.format("JobsRepository.save(%s)", job),
                        this::addJob),
                Pair.of(
                        "JobsRepository.findAll()",
                        this::getAllJobs),
                Pair.of(
                        jobId -> String.format("JobsRepository.findById(\"%s\")", jobId),
                        this::getJob),
                Pair.of(
                        filter -> String.format("JobsRepository.findAllSectionsByFilters(%s,%s,%s,%s)",
    ...
    public @Nonnull List<SectionPOJO> findAllSectionsByFilters(
        ...
        return jobsRepository.findAllSectionsByFilters(
                    filter.jobId,
                    filter.sectionName,
                    filter.geoClassName,
                    filter.geoClassCode);
